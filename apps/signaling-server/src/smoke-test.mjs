...
const peers = ["agent", "controller"].map((role) => new WebSocket(url).on("open", function () {
  this.send(JSON.stringify({ type: "hello", sessionId, joinToken: token, role }));
}));
...
await new Promise((resolve, reject) => {
  ...
  const timer = setTimeout(() => reject(new Error("invalid password was not rejected")), 5_000);
