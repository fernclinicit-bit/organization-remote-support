...
try {
  ...
  const agent = await open("agent", sessionId, "047984"); sockets.push(agent);
  const agentReady = waitForMessage(agent, "peer-ready");
  const controller = await open("controller", sessionId, "047984"); sockets.push(controller);
  ...
  console.log("PASS: pairing, relay metadata, signaling relay, invalid password, offline-agent handling, and server-only message rejection");
} finally {
